...
/* Controllers */
var homeControllers = angular.module('articlesControllers', [])

//Controller for settings page
    .controller('MainController',
        function ($scope,$rootScope, $window, $routeParams, $location, $http, ModalService, articlesService,chartsManager, utilityService) {

            $rootScope.openChildTab = [];
            $rootScope.openChildTab[$routeParams.menuId] = true;
            $scope.showDataCriteria = true;

            articlesService.checkSession();
            ...
            $scope.getReportTable = function () {
                ...
                articlesService.getReportTables().then(function(reportTables){

                    $scope.analysis = articlesService.prepareLeftMenu(reportTables.reportTables);
            ...
            $scope.loadAnalytics = function (url) {
                ...
                articlesService.getAnalytics(url).then(function(analytics){
                    $scope.analyticsObject = analytics;
                    $scope.dataDimension = articlesService.getDataDimension(analytics,$scope.dataArray);
                    $scope.categoryDimension = articlesService.setSelectedCategory([{name:"Administrative Units",id:"ou"},{name:"Period",id:"pe"}],$routeParams.category);
                    $scope.chartObject = {};
            ...
            articlesService.loggedUser().then(function (results) {
                ...
                results.organisationUnits.forEach(function (orgUnit) {
                    articlesService.sortOrganisationUnits(orgUnit, $scope.orgUnitArray);
                });
            ...
            $scope.loadMessages = function () {
                $scope.messages = [];
                articlesService.getMessages().then(function (response) {
            ...
            $scope.loadCharts = function () {
                return articlesService.loadChartStorage();
            }
    ...
    .controller('articlesController', function ($scope, $window, $routeParams, $location, $filter, articlesService, chartsManager,utilityService) {
        ...
        $scope.loadTabs = function () {
            articlesService.getTabs().then(function (response) {
                $scope.tabs = response;
            });
        }
        ...
        $scope.loadArticles = function () {
            articlesService.getTabContent().then(function (response) {
        ...
        $scope.getReportTable = function () {
            articlesService.getReportTables().then(function (reportTables) {
                ...
                var menuarr = [{'name': "Agriculture", values: []}, {
                    'name': "Livestock",
                    values: []
                }, {'name': "Fishery", values: []}, {'name': "Trade", values: []}, {
        ...
        $scope.loadCharts = function () {
            articlesService.getSelectedCharts().then(function (response) {
                $scope.charts = response;
        ...
        $scope.loadInformations = function () {
             ...
             * */
            articlesService.loadInformations().then(function (response) {
    ...
    .controller('analysisController', function ($scope, $window, $routeParams, $location, $filter, chartsManager,articlesService, utilityService, $rootScope) {
        ...
        $scope.loadRawCharts = function () {
            articlesService.getCharts().then(function (response) {
                var rowcharts = response.charts;
                $scope.loadCharts().then(function (response) {
                    if (response) {

                        if (response.length == rowcharts.length) {
                            $scope.charts = response;//articlesService.getSelectedCharts(response.chartsStorage);
                        } else {
                            articlesService.saveCharts(rowcharts);
                            if (response.length > 0) {
                                articlesService.updateCharts(rowcharts);
                            }
                        }
    ...
    .controller('analysisPeriodController', function ($scope, $window, $routeParams, $location, $filter, articlesService, utilityService) {
    ...
    .controller('analysisDataController', function ($scope, $window, $routeParams, $location, $filter, articlesService, chartsManager, utilityService, $rootScope) {
        ...
        $scope.orgunitCallBack = function (item, selectedItems, selectedType) {
            ...
            var criteriaArray = articlesService.getSelectionCriterias(item, selectedItems, selectedType, $location.path());
        ...
        $scope.loadAnalytics = function (url) {
            ...
            articlesService.getAnalytics(url).then(function (analytics) {
                $scope.analyticsObject = analytics;
                $scope.dataDimension = articlesService.getDataDimension(analytics, $scope.dataArray);
                $scope.categoryDimension = articlesService.setSelectedCategory([{
        ...
        articlesService.loggedUser().then(function (results) {
            articlesService.getOrgUnitTree(results.organisationUnits)
